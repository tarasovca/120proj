...
public class AgentsGUI extends GUIStateSweep {
			...
			public void draw(Object object, java.awt.Graphics2D graphics, sim.portrayal.DrawInfo2D info) {
				...
				if (agent.inQuarantine) {
					...
					graphics.drawOval((int) (info.draw.x - info.draw.width / 2.0),
							(int) (info.draw.y - info.draw.height / 2.0), (int) info.draw.width,
	...
	public static void main(String[] args) {
		...
		System.out.println("Starting simulation and initializing chart.");
		String[] titles = { "Infected Agents" };
		String[] xLabels = { "Time Steps" };
		String[] yLabels = { "Count" };

		initializeArrayTimeSeriesChart(1, titles, xLabels, yLabels);

		// Initialize the MASON simulation with the specified environment, experimenter,
		// GUI classes, and dimensions
		AgentsGUI.initialize(Environment.class, Experimenter.class, AgentsGUI.class, 400, 400, Color.WHITE, Color.RED,
				true, Spaces.SPARSE);
	}
}
